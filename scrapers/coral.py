"""
Coral Travel Estonia (coral.ee) — Playwright.
"""
import logging, json, re
logger = logging.getLogger(__name__)
SITE = "https://www.coral.ee"

def fetch_coral() -> list:
    try:
    ...
    with sync_playwright() as p:
        ...
        for url in [f"{SITE}/ru/last-minute", f"{SITE}/ru/egypt", f"{SITE}/ru/hurgada", f"{SITE}/ru/sharm"]:
            try:
                page.goto(url, wait_until="domcontentloaded", timeout=30000)
                page.wait_for_timeout(5000)
                page.evaluate("window.scrollTo(0, document.body.scrollHeight / 2)")
                page.wait_for_timeout(2000)
            except Exception as e: logger.debug(f"Coral {url}: {e}")

        try: tours.extend(_parse_html(page.content(), "Coral Travel"))
        except Exception: pass
        browser.close()

    for data in captured: tours.extend(_parse(data, "Coral Travel"))
    unique = _dedupe(tours)
    (logger.info if unique else logger.warning)(f"Coral Travel: {len(unique) if unique else 'туры не найдены'}")
    return unique
...
def _list(data):
    ...
    if isinstance(data, dict):
        for k in ("packages","tours","offers","results","data","items","hotels","products"):
            v = data.get(k)
